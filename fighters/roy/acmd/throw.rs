...
unsafe extern "C" fn game_roy_throwf(agent: &mut L2CAgentBase) {
...
unsafe extern "C" fn effect_roy_throwf(agent: &mut L2CAgentBase) {
    frame(agent.lua_state_agent, 15.0);
    if macros::is_excute(agent) {
...
unsafe extern "C" fn sound_roy_throwf(agent: &mut L2CAgentBase) {
...
unsafe extern "C" fn expression_roy_throwf(agent: &mut L2CAgentBase) {
    if macros::is_excute(agent) {
...
pub fn install() {
    Agent::new("roy")
    .acmd("game_throwf", game_roy_throwf, Priority::Low)
    .acmd("effect_throwf", effect_roy_throwf, Priority::Low)
    .acmd("sound_throwf", sound_roy_throwf, Priority::Low)
    .acmd("expression_throwf", expression_roy_throwf, Priority::Low)
    .install();
}