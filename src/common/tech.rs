...
unsafe extern "C" fn waveland_plat_drop(fighter: &mut L2CFighterCommon) {
	unsafe {
		let boma = smash::app::sv_system::battle_object_module_accessor(fighter.lua_state_agent);
		let status_kind = StatusModule::status_kind(boma);
		...
		if status_kind == *FIGHTER_STATUS_KIND_LANDING && plat_drop_statuses.contains(&prev_status) {
			...
			if WorkModule::is_flag(boma, *FIGHTER_INSTANCE_WORK_ID_FLAG_ENABLE_WAVELAND_PLATDROP) {
		...
		if reset_statuses.contains(&status_kind) {
			...
			if ControlModule::get_stick_y(boma) > -0.3 {
				WorkModule::on_flag(boma, *FIGHTER_INSTANCE_WORK_ID_FLAG_ENABLE_WAVELAND_PLATDROP);
			}
		}
	}
}*/
