...
use crate::FIGHTER_INSTANCE_WORK_ID_INT_CUSTOMIZE_SPECIAL_HI_NO;
use crate::FIGHTER_KINETIC_ENERGY_ID_CONTROL;
use crate::KineticModule;
use crate::app::KineticEnergy;
use crate::ENERGY_GRAVITY_RESET_TYPE_GRAVITY;
use crate::ENERGY_STOP_RESET_TYPE_AIR;
...
pub unsafe extern "C" fn special_s_init(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    if [*FIGHTER_STATUS_KIND_SPECIAL_S, *FIGHTER_ROY_STATUS_KIND_SPECIAL_S2].contains(&current_status_kind) {
        if current_situation_kind == *SITUATION_KIND_GROUND {
            ...
            if !WorkModule::is_flag(fighter.module_accessor, FIGHTER_INSTANCE_WORK_ID_SPECIAL_STALL_USED) {
                WorkModule::on_flag(fighter.module_accessor, FIGHTER_INSTANCE_WORK_ID_SPECIAL_STALL_USED);
