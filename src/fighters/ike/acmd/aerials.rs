...
unsafe extern "C" fn game_ike_hiair(agent: &mut L2CAgentBase) {
...
unsafe extern "C" fn effect_ike_hiair(agent: &mut L2CAgentBase) {
...
unsafe extern "C" fn sound_ike_hiair(agent: &mut L2CAgentBase) {
    frame(agent.lua_state_agent, 11.0);
    if macros::is_excute(agent) {
...
unsafe extern "C" fn expression_ike_hiair(agent: &mut L2CAgentBase) {
    frame(agent.lua_state_agent, 11.0);
    if macros::is_excute(agent) {
...
pub fn install() {
    Agent::new("ike")
    .acmd("game_attackairhi", game_ike_hiair, Priority::Low)
    .acmd("effect_attackairhi", effect_ike_hiair, Priority::Low)
    .acmd("sound_attackairhi", sound_ike_hiair, Priority::Low)
    .acmd("expression_attackairhi", expression_ike_hiair, Priority::Low)
    .install();
}